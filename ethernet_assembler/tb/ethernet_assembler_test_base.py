...
from ethernet_assembler.tb.ethernet_assembler_driver import EthernetAssemblerDriver
...
class EthernetAssemblerTestBase(GenericTestBase):
    def __init__(
        self,
        dut,
        driver=EthernetAssemblerDriver,
        ...
        if cycle_accurate_monitor:
            if monitor is GenericValidMonitor:
                monitor = GenericMonitor
            if isinstance(model, type) and issubclass(model, EthernetAssemblerModel):
                model_factory = lambda: model(cycle_accurate=True)
