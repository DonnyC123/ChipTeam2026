module tx_scheduling #(
    ...
    input  logic [NUM_QUEUES-1:0] q_packet_ready_i,
    input  logic                  fifo_full_i,
    input  logic                  fifo_grant_i,
    output logic                  dma_read_en_o,
    output logic [QID_W-1:0]      dma_queue_sel_o,
  ...
  // - q_packet_ready_i[n]=1: queue n has a complete frame buffered and may be
  //   selected from IDLE. Once selected, the scheduler keeps serving that queue
  //   until q_last_i is observed or MAX_BURST_BEATS forces a rotation.
  ...
  // - If the selected queue unexpectedly deasserts q_valid_i mid-packet, the
  //   scheduler returns to IDLE so other packet-ready queues are not blocked.
  ...
  initial begin
    if (NUM_QUEUES < 1) begin
      $fatal(1, "tx_scheduling: NUM_QUEUES must be >= 1");
    end
  ...
  // Round-robin priority scan starting from last_served + 1. A queue is eligible
  // to start service only when it has both a valid head word and a complete
  // packet buffered behind that head word.
  always_comb begin : rr_arbiter
    next_found = 1'b0;
    next_queue = '0;
    for (int unsigned i = 0; i < NUM_QUEUES; i++) begin
      rr_raw_idx = (int'(last_served_q) + 1 + i) % NUM_QUEUES;
      rr_cand    = QID_W'(rr_raw_idx);
      if (!next_found && q_valid_i[rr_cand] && q_packet_ready_i[rr_cand]) begin
        next_queue = rr_cand;
        next_found = 1'b1;
      end
    end
    ...
    case (state_q)
      ...
      IDLE: begin
        ...
        if (!fifo_full_i && next_found) begin
          ...
          if (fifo_grant_i) begin
            dma_read_en_d = 1'b1;
            if (q_last_i[next_queue] || (MAX_BURST_BEATS == 1)) begin
              last_served_d = next_queue;
              burst_cnt_d   = '0;
            end else begin
              state_d     = SERVING;
              burst_cnt_d = BURST_CNT_W'(1);
            end
          end
        end
      end
      ...
      SERVING: begin
        if (!fifo_full_i) begin
          if (!q_valid_i[queue_sel_q]) begin
            state_d       = IDLE;
...
`ifndef SYNTHESIS
  ...
  property p_idle_request_requires_packet_ready;
    @(posedge clk) disable iff (rst)
      ((state_q == IDLE) && fifo_req_o) |->
        (q_valid_i[dma_queue_sel_o] && q_packet_ready_i[dma_queue_sel_o]);
  endproperty
  a_idle_request_requires_packet_ready: assert property (p_idle_request_requires_packet_ready);
  ...
  property p_watchdog_forces_idle_after_max_burst;
    @(posedge clk) disable iff (rst)
      (state_q == SERVING &&
       ...
       (burst_cnt_q == BURST_CNT_W'(MAX_BURST_BEATS - 1)))
      |=> (state_q == IDLE);
  endproperty
  a_watchdog_forces_idle_after_max_burst: assert property (p_watchdog_forces_idle_after_max_burst);

  property p_mid_packet_empty_releases_scheduler;
    @(posedge clk) disable iff (rst)
      (state_q == SERVING && !fifo_full_i && !q_valid_i[queue_sel_q])
      |=> (state_q == IDLE);
  endproperty
  a_mid_packet_empty_releases_scheduler: assert property (p_mid_packet_empty_releases_scheduler);
