...
NUM_QUEUES = TxSchedulingSequenceItem.NUM_QUEUES
...
class TxSchedulingSequence(GenericSequence):
    ...
    async def add_beat(
        self,
        q_valid=0, q_last=0,
        fifo_full=False, fifo_grant=True,
    ):
        """Drive one scheduler cycle and notify model."""
        await self.notify_subscribers({
            "q_valid": q_valid, "q_last": q_last,
            "fifo_full": fifo_full, "fifo_grant": fifo_grant,
        })
        await self.add_transaction(
            TxSchedulingSequenceItem(
                q_valid_i=LogicArray.from_unsigned(q_valid, NUM_QUEUES),
                q_last_i=LogicArray.from_unsigned(q_last, NUM_QUEUES),
