...
class PCSMonitor(GenericValidMonitor[PCSOutputBlockTransaction]):
    ...
    def _process_block(self, decoded_block: DecodedPCSBlock) -> Optional[bytes]:
        ...
        if self._awaiting_post_eof_idle:
            if not decoded_block.is_idle:
                raise RuntimeError(
                    f"Frame {self._frame_index} ended, next block must be IDLE_BLK; "
    ...
    def _process_initial_outside_frame_block(
        ...
        raise RuntimeError(
            f"Before any frame starts, only IDLE_BLK/SOF_L0/SOF_L4 are legal; "
            f"saw {decoded_block.name}"
        )
    ...
    def _process_post_eof_block(
        ...
        if decoded_block.is_sof_l4:
            if self._idle_blocks_since_eof < 1:
                raise RuntimeError("SOF_L4 is illegal before mandatory post-EOF idle")
        ...
        if decoded_block.is_sof_l0:
            if self._idle_blocks_since_eof < 1:
                raise RuntimeError("SOF_L0 is illegal before mandatory post-EOF idle")
            self._start_frame(decoded_block)
            return None

        raise RuntimeError(
            f"After EOF, only IDLE_BLK/SOF_L4/SOF_L0 are legal; saw {decoded_block.name}"
        )
    ...
    def _process_in_frame_block(
        ...
        raise RuntimeError(
            f"Frame {self._frame_index} may only emit DATA_HDR/TERM_L* once EOF is legal; "
            f"saw {decoded_block.name}"
        )
    ...
    def assert_complete(self) -> None:
        if self._in_frame:
            raise RuntimeError(
                ...
                f"with {len(self._current_frame_payload)} payload bytes"
            )
        if self._awaiting_post_eof_idle:
            raise RuntimeError(
                f"Monitor stopped before mandatory post-EOF idle after frame "
