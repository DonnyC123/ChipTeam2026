module scrambler #(
    ...
    input  logic                     clk,
    input  logic                     rst,
    input  logic [BIT_IN_W-1:0]      x_64b_i,
    input  logic                     valid_i,
    input  logic [HEAD_W-1:0]        x_2b_header_i,
    output logic [BIT_OUT_W-1:0]     x_66b_o,
...
always_comb begin
    ...
    if (valid_i) begin
        header_prop_d = x_2b_header_i;
        for (int i = 0; i < BIT_IN_W; i++) begin
            scrambled_d[i]     =x_64b_i[i] ^ state_intermediate[TAP_1] ^ state_intermediate[TAP_2];
            state_intermediate = {state_intermediate[STATE_W-2:0], (x_64b_i[i] ^ state_intermediate[TAP_1] ^ state_intermediate[TAP_2])};
        end
...
assign x_66b_o        = {header_prop_q[HEAD_W-1:0], scrambled_q[BIT_IN_W-1:0]};
